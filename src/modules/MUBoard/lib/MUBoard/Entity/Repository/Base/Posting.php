 ...
 */
class MUBoard_Entity_Repository_Base_Posting extends EntityRepository
{
    /**
    ...
    public function truncateTable()
    {
        $query = $this->getEntityManager()
                 ->createQuery('DELETE MUBoard_Entity_Posting');
        $query->execute();
    }
    ...
    public function selectSearch($fragment = '', $exclude = array(), $orderBy = '', $currentPage = 1, $resultsPerPage = 25, $useJoins = true)
    {
        ...
        $whereSub .= ((!empty($whereSub)) ? ' OR ' : '') . 'tbl.invocations = \'' . $fragment . '\'';
        $whereSub .= ((!empty($whereSub)) ? ' OR ' : '') . 'tbl.firstImage = \'' . $fragment . '\'';
        $whereSub .= ((!empty($whereSub)) ? ' OR ' : '') . 'tbl.secondImage = \'' . $fragment . '\'';
        $whereSub .= ((!empty($whereSub)) ? ' OR ' : '') . 'tbl.thirdImage = \'' . $fragment . '\'';
        $whereSub .= ((!empty($whereSub)) ? ' OR ' : '') . 'tbl.firstFile = \'' . $fragment . '\'';
        $whereSub .= ((!empty($whereSub)) ? ' OR ' : '') . 'tbl.secondFile = \'' . $fragment . '\'';
        $whereSub .= ((!empty($whereSub)) ? ' OR ' : '') . 'tbl.thirdFile = \'' . $fragment . '\'';
     ...
     * @param int    $excludeid  Id of postings to exclude (optional).
