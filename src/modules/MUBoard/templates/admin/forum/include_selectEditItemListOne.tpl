{* purpose of this template: inclusion template for display of related Forum in admin area *}
...
{assign var='idPrefixItem' value="`$idPrefix`Reference_`$item.id`"}
    <li id="{$idPrefixItem}">
    {$item.title}
 <a id="{$idPrefixItem}Edit" href="{modurl modname='MUBoard' type='admin' func='edit' ot='forum' id=$item.id}">{$editImage}</a>
