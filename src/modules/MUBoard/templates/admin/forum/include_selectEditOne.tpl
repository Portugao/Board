{* purpose of this template: inclusion template for managing related Forums in admin area *}
<fieldset>
    ...
    <div class="muboardRelationRightSide">
        ...
        <div id="{$idPrefix}AddFields">
            ...
            <a id="{$idPrefix}SelectorDoNew" href="{modurl modname='MUBoard' type='admin' func='edit' ot='forum'}" title="{gt text='Create new forum'}" class="z-button muboardInlineButton">{gt text='Create'}</a>
        </div>
    ...
    <div class="muboardRelationLeftSide">
        {if isset($userSelection.$aliasName) && $userSelection.$aliasName ne ''}
            ...
            {include file='admin/forum/include_selectEditItemListOne.tpl' item=$userSelection.$aliasName}
        {elseif $mode ne 'create' || isset($relItem.$aliasName)}
            {include file='admin/forum/include_selectEditItemListOne.tpl' item=$relItem.$aliasName}
        {else}
            {include file='admin/forum/include_selectEditItemListOne.tpl'}
        {/if}
    </div>
    <br style="clear: both" />
    </div>
