{* purpose of this template: ranks display view in admin area *}
...
<div class="z-admin-content-pagetitle">
    ...
    <h3>{$templateTitle|notifyfilters:'muboard.filter_hooks.ranks.filter'}</h3>
</div>
...
{if isset($rank.user) && $rank.user ne null}
    {include file='admin/user/include_displayItemListMany.tpl' items=$rank.user}
{/if}
...
<p class="manageLink">
    ...
    <a href="{modurl modname='MUBoard' type='admin' func='edit' ot='user' rank="`$rank.id`" returnTo='adminDisplayRank'}" title="{$createTitle}" class="z-icon-es-add">
        {$createTitle}
    </a>
</p>
{/if}
...
<noscript><div id="noscriptspecial{$itemid}">
    ...
    {include file='admin/include_standardfields_display.tpl' obj=$rank}
