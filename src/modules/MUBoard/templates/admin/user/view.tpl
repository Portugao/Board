...
<div class="z-admin-content-pagetitle">
    ...
    {checkpermissionblock component='MUBoard::' instance='.*' level="ACCESS_ADD"}
        ...
        <a href="{modurl modname='MUBoard' type='admin' func='edit' ot='user'}" title="{$createTitle}" class="z-icon-es-add">
            {$createTitle}
        </a>
    ...
    {if isset($showAllEntries) && $showAllEntries eq 1}
        ...
        <a href="{modurl modname='MUBoard' type='admin' func='view' ot='user'}" title="{$linkTitle}" class="z-icon-es-view">
            {$linkTitle}
        </a>
        {assign var='all' value=1}
    {else}
        {gt text='Show all entries' assign='linkTitle'}
        <a href="{modurl modname='MUBoard' type='admin' func='view' ot='user' all=1}" title="{$linkTitle}" class="z-icon-es-view">
            {$linkTitle}
        </a>
    {/if}
...
<table class="z-datatable">
    ...
    <tr>
        <th id="huserid" scope="col" class="z-right">
            {sortlink __linktext='Userid' sort='userid' currentsort=$sort sortdir=$sdir all=$all modname='MUBoard' type='admin' func='view' ot='user'}
        </th>
        ...
        <th id="hrank" scope="col" class="z-left">
            {sortlink __linktext='Rank' sort='rank' currentsort=$sort sortdir=$sdir all=$all modname='MUBoard' type='admin' func='view' ot='user'}
        </th>
    ...
    <tr class="{cycle values='z-odd, z-even'}">
        ...
        <td headers="hrank" class="z-left">
            {if isset($user.Rank) && $user.Rank ne null}
                <a href="{modurl modname='MUBoard' type='admin' func='display' ot='rank' id=$user.Rank.id}">
                    {$user.Rank.name|default:""}
                </a>
                <a id="rankItem{$user.id}_rel_{$user.Rank.id}Display" href="{modurl modname='MUBoard' type='admin' func='display' ot='rank' id=$user.Rank.id theme='Printer'}" title="{gt text='Open quick view window'}" style="display: none">
