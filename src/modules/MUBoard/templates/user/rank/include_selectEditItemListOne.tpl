{* purpose of this template: inclusion template for display of related Rank in user area *}
...
{assign var='idPrefixItem' value="`$idPrefix`Reference_`$item.id`"}
    <li id="{$idPrefixItem}">
    {$item.name}
 <a id="{$idPrefixItem}Edit" href="{modurl modname='MUBoard' type='user' func='edit' ot='rank' id=$item.id}">{$editImage}</a>
