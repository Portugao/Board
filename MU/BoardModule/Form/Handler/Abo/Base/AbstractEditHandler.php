...
namespace MU\BoardModule\Form\Handler\Abo\Base;

use MU\BoardModule\Form\Handler\Common\EditHandler;
use MU\BoardModule\Form\Type\AboType;
...
abstract class AbstractEditHandler extends EditHandler
{
    ...
    {
        $this->objectType = 'abo';
        $this->objectTypeCapital = 'Abo';
        $this->objectTypeLower = 'abo';
        ...
        if ('create' == $this->templateParameters['mode']) {
            if (!$this->modelHelper->canBeCreated($this->objectType)) {
                $this->requestStack->getCurrentRequest()->getSession()->getFlashBag()->add('error', $this->__('Sorry, but you can not create the abo yet as other items are required which must be created before!'));
    ...
    {
        return $this->formFactory->create(AboType::class, $this->entityRef, $this->getFormOptions());
    }
    ...
    protected function getRedirectCodes()
    {
        ...
        // user index page of abo area
        $codes[] = 'userIndex';
        // admin index page of abo area
        $codes[] = 'adminIndex';
        // user list of abos
        $codes[] = 'userView';
        // admin list of abos
        $codes[] = 'adminView';
        // user list of own abos
        $codes[] = 'userOwnView';
        // admin list of own abos
        $codes[] = 'adminOwnView';
        // user detail page of treated abo
        $codes[] = 'userDisplay';
        // admin detail page of treated abo
        $codes[] = 'adminDisplay';
    ...
    protected function getDefaultReturnUrl(array $args = [])
    {
        ...
        if ($objectIsPersisted) {
            // redirect to the detail page of treated abo
    ...
    protected function getDefaultMessage(array $args = [], $success = false)
    {
        ...
        switch ($args['commandName']) {
            case 'submit':
                if ('create' == $this->templateParameters['mode']) {
                    $message = $this->__('Done! Abo created.');
                } else {
                    $message = $this->__('Done! Abo updated.');
                }
                break;
            case 'delete':
                $message = $this->__('Done! Abo deleted.');
                break;
            default:
                $message = $this->__('Done! Abo updated.');
                break;
    ...
    public function applyAction(array $args = [])
    {
        ...
        try {
            ...
            $logArgs = ['app' => 'MUBoardModule', 'user' => $this->currentUserApi->get('uname'), 'entity' => 'abo', 'id' => $entity->getKey(), 'errorMessage' => $exception->getMessage()];
