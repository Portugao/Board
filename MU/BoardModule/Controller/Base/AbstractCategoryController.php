...
/**
 * Category controller base class.
 */
abstract class AbstractCategoryController extends AbstractController
{
    ...
    protected function viewInternal(Request $request, $sort, $sortdir, $pos, $num, $isAdmin = false)
    {
        ...
        $sortableColumns = new SortableColumns($this->get('router'), 'muboardmodule_category_' . ($isAdmin ? 'admin' : '') . 'view', 'sort', 'sortdir');
        ...
        foreach ($templateParameters['items'] as $category) {
            if (!$permissionHelper->hasEntityPermission($category, $permLevel)) {
                continue;
            }
     ...
     */
    protected function displayInternal(Request $request, CategoryEntity $category, $isAdmin = false)
    {
        $objectType = 'category';
        // permission check
        $permLevel = $isAdmin ? ACCESS_ADMIN : ACCESS_READ;
    ...
    protected function editInternal(Request $request, $isAdmin = false)
    {
        ...
        $formHandler = $this->get('mu_board_module.form.handler.category');
    ...
    protected function deleteInternal(Request $request, CategoryEntity $category, $isAdmin = false)
    {
        ...
        $logArgs = ['app' => 'MUBoardModule', 'user' => $this->get('zikula_users_module.current_user')->get('uname'), 'entity' => 'category', 'id' => $category->getKey()];
        
        // determine available workflow actions
        $workflowHelper = $this->get('mu_board_module.workflow_helper');
        $actions = $workflowHelper->getActionsForObject($category);
        ...
        $redirectRoute = 'muboardmodule_category_' . ($isAdmin ? 'admin' : '') . 'view';
        ...
        if (!$deleteAllowed) {
            $this->addFlash('error', $this->__('Error! It is not allowed to delete this category.'));
    ...
    protected function handleSelectedEntriesActionInternal(Request $request, $isAdmin = false)
    {
        ...
        foreach ($items as $itemId) {
            ...
            try {
                ...
                $logger->error('{app}: User {user} tried to execute the {action} workflow action for the {entity} with id {id}, but failed. Error details: {errorMessage}.', ['app' => 'MUBoardModule', 'user' => $userName, 'action' => $action, 'entity' => 'category', 'id' => $itemId, 'errorMessage' => $exception->getMessage()]);
            }
            ...
            if ($action == 'delete') {
                $this->addFlash('status', $this->__('Done! Item deleted.'));
                $logger->notice('{app}: User {user} deleted the {entity} with id {id}.', ['app' => 'MUBoardModule', 'user' => $userName, 'entity' => 'category', 'id' => $itemId]);
            } else {
