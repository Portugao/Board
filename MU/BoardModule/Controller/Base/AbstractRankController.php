...
abstract class AbstractRankController extends AbstractController
{
    ...
    protected function viewInternal(Request $request, $sort, $sortdir, $pos, $num, $isAdmin = false)
    {
        ...
        $filteredEntities = [];
        foreach ($templateParameters['items'] as $rank) {
            if (!$permissionHelper->hasEntityPermission($rank, $permLevel)) {
                continue;
            }
            $filteredEntities[] = $rank;
        }
     ...
     *
     * @param Request $request Current request instance
     * @param integer $id Identifier of treated rank instance
     *
     * @return Response Output
     *
     * @throws AccessDeniedException Thrown if the user doesn't have required permissions
     * @throws NotFoundHttpException Thrown if rank to be displayed isn't found
     */
    public function adminDisplayAction(Request $request, $id)
    {
        $rank = $this->get('mu_board_module.entity_factory')->getRepository('rank')->selectById($id);
        if (null === $rank) {
            throw new NotFoundHttpException($this->__('No such rank found.'));
        }
    ...
    protected function displayInternal(Request $request, RankEntity $rank, $isAdmin = false)
    {
        ...
        $controllerHelper = $this->get('mu_board_module.controller_helper');
        $templateParameters = $controllerHelper->processDisplayActionParameters($objectType, $templateParameters, $rank->supportsHookSubscribers());
     ...
     *
     * @throws AccessDeniedException Thrown if the user doesn't have required permissions
     * @throws NotFoundHttpException Thrown if rank to be deleted isn't found
     * @throws RuntimeException      Thrown if another critical error occurs (e.g. workflow actions not available)
     */
    ...
    protected function deleteInternal(Request $request, $id, $isAdmin = false)
    {
        ...
        $form = $this->createForm(DeletionType::class, $rank);
