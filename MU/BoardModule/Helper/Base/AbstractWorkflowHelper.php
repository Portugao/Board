...
abstract class AbstractWorkflowHelper
{
    ...
    public function collectAmountOfModerationItems()
    {
        ...
        $state = 'waiting';
        $objectType = 'posting';
        if ($this->permissionApi->hasPermission('MUBoardModule:' . ucfirst($objectType) . ':', '::', ACCESS_ADD)) {
            $amount = $this->getAmountOfModerationItems($objectType, $state);
            if ($amount > 0) {
                $amounts[] = [
                    'aggregateType' => 'postingsApproval',
                    'description' => $this->translator->__('Postings pending approval'),
                    'amount' => $amount,
                    'objectType' => $objectType,
                    'state' => $state,
                    'message' => $this->translator->_fn('One posting is waiting for approval.', '%amount% postings are waiting for approval.', $amount, ['%amount%' => $amount])
                ];
        
                $this->logger->info('{app}: There are {amount} {entities} waiting for approval.', ['app' => 'MUBoardModule', 'amount' => $amount, 'entities' => 'postings']);
            }
        }
