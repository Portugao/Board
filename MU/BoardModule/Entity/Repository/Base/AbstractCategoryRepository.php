...
namespace MU\BoardModule\Entity\Repository\Base;
 ...
 *
 * This is the base repository class for category entities.
 */
abstract class AbstractCategoryRepository extends EntityRepository
{
    /**
     * @var string The main entity class
     */
    protected $mainEntityClass = 'MU\BoardModule\Entity\CategoryEntity';
    ...
    public function updateCreator($userId, $newUserId, TranslatorInterface $translator, LoggerInterface $logger, CurrentUserApiInterface $currentUserApi)
    {
        ...
        $logArgs = ['app' => 'MUBoardModule', 'user' => $currentUserApi->get('uname'), 'entities' => 'categories', 'userid' => $userId];
     ...
     *
     * @return array|categoryEntity Retrieved data array or categoryEntity instance
     */
     ...
     * @param boolean $slimMode If activated only some basic fields are selected without using any joins (optional) (default=false)
     *
     * @return ArrayCollection Collection containing retrieved categoryEntity instances
     */
    public function selectByIdList($idList = [0], $useJoins = true, $slimMode = false)
    {
        ...
        if (!$slimMode && null !== $this->collectionFilterHelper) {
            $qb = $this->collectionFilterHelper->applyDefaultFilters('category', $qb);
        }
    ...
    public function getListQueryBuilder($where = '', $orderBy = '', $useJoins = true, $slimMode = false)
    {
        ...
        if (!$slimMode && null !== $this->collectionFilterHelper) {
            $qb = $this->collectionFilterHelper->addCommonViewFilters('category', $qb);
        }
    ...
    public function selectSearch($fragment = '', array $exclude = [], $orderBy = '', $currentPage = 1, $resultsPerPage = 25, $useJoins = true)
    {
        ...
        if (null !== $this->collectionFilterHelper) {
            $qb = $this->collectionFilterHelper->addSearchFilter('category', $qb, $fragment);
        }
    ...
    {
        $selection = 'COUNT(tbl.id) AS numCategories';
    ...
    public function selectCount($where = '', $useJoins = false, array $parameters = [])
    {
        ...
        if (null !== $this->collectionFilterHelper) {
            $qb = $this->collectionFilterHelper->applyDefaultFilters('category', $qb, $parameters);
        }
     ...
     * @param integer $excludeId  Id of categories to exclude (optional)
     *
    ...
    {
        $qb->leftJoin('tbl.forum', 'tblForum');
