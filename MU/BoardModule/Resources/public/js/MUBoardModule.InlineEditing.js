 ...
 */
function mUBoardCreateInlineEditingWindowInstance(containerElem)
{
    var newWindowId;
 ...
 */
function mUBoardInitInlineEditingWindow(objectType, idPrefix, containerId, inputType)
{
    ...
    jQuery.each(mUBoardInlineEditHandlers, function (key, editHandler) {
        ...
        editHandler.windowInstanceId = mUBoardCreateInlineEditingWindowInstance(jQuery('#' + containerId));
    });
    ...
    newEditHandler = {
        ...
        inputType: inputType,
        windowInstanceId: mUBoardCreateInlineEditingWindowInstance(jQuery('#' + containerId))
    };

    // add it to the list of edit handlers
    mUBoardInlineEditHandlers.push(newEditHandler);
}

/**
 * Creates a link for editing an existing item using inline editing.
 */
function mUBoardCreateInlineEditLink(objectType, idPrefix, elemPrefix, itemId)
{
 ...
 */
function mUBoardInitInlineEditLink(objectType, idPrefix, elemPrefix, itemId, inputType)
{
    jQuery('#' + elemPrefix + 'Edit').click(function (event) {
        event.preventDefault();
        mUBoardInitInlineEditingWindow(objectType, idPrefix, idPrefix + 'Reference_' + itemId + 'Edit');
    });
}
 ...
 */
function mUBoardDetermineInputReference(objectType, alias, idPrefix, inputType, targetWindow)
{
    var inputPrefix, inputIdentifier, inputField;

    // determine reference to input element
    inputPrefix = targetWindow.jQuery('.muboard-edit-form').first().attr('name');
    inputField = null;
 ...
 */
function mUBoardInitInlineEditingButtons(objectType, alias, idPrefix, inputType, createUrl)
{
    var inputReference, createButtonId, createButton, itemIds, itemIdsArr;

    inputReference = mUBoardDetermineInputReference(objectType, alias, idPrefix, inputType, window);
    ...
    if (jQuery('#' + createButtonId).length < 1) {
        ...
        createButton = jQuery('<a />', {
            ...
            class: 'btn btn-default muboardmodule-inline-button'
        }).append(
    ...
    createButton.click(function (event) {
        event.preventDefault();
        mUBoardInitInlineEditingWindow(objectType, idPrefix, createButtonId, inputType);
    });
    ...
    jQuery.each(itemIdsArr, function (key, existingId) {
        ...
        if (existingId) {
            ...
            jQuery('#' + elemPrefix).click(function (event) {
                event.preventDefault();
                mUBoardInitInlineEditingWindow(objectType, idPrefix, elemPrefix, inputType);
            });
        }
    });
}
 ...
 */
function mUBoardCloseWindowFromInside(idPrefix, itemId, formattedTitle, searchTerm)
{
    ...
    jQuery.each(window.parent.mUBoardInlineEditHandlers, function (key, editHandler) {
        ...
        inputReference = mUBoardDetermineInputReference(editHandler.objectType, editHandler.alias, idPrefix, inputType, window.parent);
        if (null === inputReference || null === inputReference.field) {
            return;
        }
        ...
        window.parent.mUBoardSimpleAlert(anchorElement, window.parent.Translator.__('Information'), window.parent.Translator.__('Action has been completed.'), 'actionDoneAlert', 'success');
