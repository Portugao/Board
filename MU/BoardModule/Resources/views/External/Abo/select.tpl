...
{assign var='baseID' value='abo'}
<div id="itemSelectorInfo" class="hidden" data-base-id="{$baseID}" data-selected-id="{$selectedId|default:0}"></div>
<div class="row">
    <div class="col-sm-8">
        <div class="form-group">
            <label for="{$baseID}Id" class="col-sm-3 control-label">{gt text='Abo'}:</label>
            <div class="col-sm-9">
                <select id="{$baseID}Id" name="id" class="form-control">
                    {foreach item='abo' from=$items}
                        <option value="{$abo->getKey()}"{if $selectedId eq $abo->getKey()} selected="selected"{/if}>{$abo->getId()}</option>
                    {foreachelse}
        ...
        <div class="form-group">
            ...
            <div class="col-sm-9">
                <select id="{$baseID}Sort" name="sort" class="form-control">
                    <option value="userid"{if $sort eq 'userid'} selected="selected"{/if}>{gt text='Userid'}</option>
                    <option value="catid"{if $sort eq 'catid'} selected="selected"{/if}>{gt text='Catid'}</option>
                    <option value="forumid"{if $sort eq 'forumid'} selected="selected"{/if}>{gt text='Forumid'}</option>
                    <option value="postingid"{if $sort eq 'postingid'} selected="selected"{/if}>{gt text='Postingid'}</option>
    ...
    <div class="col-sm-4">
        <div id="{$baseID}Preview" style="border: 1px dotted #a3a3a3; padding: .2em .5em">
            <p><strong>{gt text='Abo information'}</strong></p>
