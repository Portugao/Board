...
{assign var='baseID' value='forum'}
<div id="itemSelectorInfo" class="hidden" data-base-id="{$baseID}" data-selected-id="{$selectedId|default:0}"></div>
<div class="row">
    <div class="col-sm-8">
        <div class="form-group">
            <label for="{$baseID}Id" class="col-sm-3 control-label">{gt text='Forum'}:</label>
            <div class="col-sm-9">
                <select id="{$baseID}Id" name="id" class="form-control">
                    {foreach item='forum' from=$items}
                        <option value="{$forum->getKey()}"{if $selectedId eq $forum->getKey()} selected="selected"{/if}>{$forum->getTitle()}</option>
                    {foreachelse}
    ...
    <div class="col-sm-4">
        <div id="{$baseID}Preview" style="border: 1px dotted #a3a3a3; padding: .2em .5em">
            <p><strong>{gt text='Forum information'}</strong></p>
