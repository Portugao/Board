{* purpose of this template: inclusion template for managing related Ranks in user area *}
<fieldset>
    ...
    <div class="muboardRelationRightSide">
        ...
        <div id="{$idPrefix}AddFields">
            ...
            <a id="{$idPrefix}SelectorDoNew" href="{modurl modname='MUBoard' type='user' func='edit' ot='rank'}" title="{gt text='Create new rank'}" class="z-button muboardInlineButton">{gt text='Create'}</a>
        </div>
    ...
    <div class="muboardRelationLeftSide">
        {if isset($userSelection.$aliasName) && $userSelection.$aliasName ne ''}
            ...
            {include file='user/rank/include_selectEditItemListOne.tpl' item=$userSelection.$aliasName}
        {elseif $mode ne 'create' || isset($relItem.$aliasName)}
            {include file='user/rank/include_selectEditItemListOne.tpl' item=$relItem.$aliasName}
        {else}
            {include file='user/rank/include_selectEditItemListOne.tpl'}
        {/if}
    </div>
    <br style="clear: both" />
    </div>
