{* purpose of this template: inclusion template for display of related Posting in user area *}
...
{assign var='idPrefixItem' value="`$idPrefix`Reference_`$item.id`"}
    <li id="{$idPrefixItem}">
    {$item.title}
 <a id="{$idPrefixItem}Edit" href="{modurl modname='MUBoard' type='user' func='edit' ot='posting' id=$item.id}">{$editImage}</a>
