{* purpose of this template: inclusion template for display of related Rank in admin area *}
...
{assign var='idPrefixItem' value="`$idPrefix`Reference_`$item.id`"}
    <li id="{$idPrefixItem}">
    {$item.name}
 <a id="{$idPrefixItem}Edit" href="{modurl modname='MUBoard' type='admin' func='edit' ot='rank' id=$item.id}">{$editImage}</a>
...
{if $item.uploadImage ne '' && isset($item.uploadImageFullPathURL)}
    <img src="{$item.uploadImageFullPathUrl|muboardImageThumb:$item.uploadImageFullPathUrl:50:40}" width="50" height="40" alt="{$item.name|replace:"\"":""}" />
{/if}
    </li>
{/if}
</ul>
