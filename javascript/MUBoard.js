 ...
 */
function muboardInitInlineWindow(containerElem, title)
{
 ...
 */
function muboardInitToggle(objectType, fieldName, itemId)
{
...
function muboardToggleFlag(objectType, fieldName, itemId)
{
    ...
    new Zikula.Ajax.Request(
        Zikula.Config.baseURL + 'ajax.php?module=MUBoard&func=toggleFlag',
        {
            method: 'post',
            parameters: pars,
            onComplete: function(req) {
                if (!req.isSuccess()) {
                    Zikula.UI.Alert(req.getMessage(), Zikula.__('Error', 'module_MUBoard'));
                    return;
                }
                var data = req.getData();
                /*if (data.message) {
                    Zikula.UI.Alert(data.message, Zikula.__('Success', 'module_MUBoard'));
                }*/
