 ...
 */
function muboardInitRelationItemsForm(objectType, idPrefix, includeEditing)
{
    // add handling for the toggle link if existing
    if ($(idPrefix + 'AddLink') != undefined) {
        $(idPrefix + 'AddLink').observe('click', function(e) { muboardToggleRelatedItemForm(idPrefix); });
    }
    // add handling for the cancel button
    if ($(idPrefix + 'SelectorDoCancel') != undefined) {
        $(idPrefix + 'SelectorDoCancel').observe('click', function(e) { muboardResetRelatedItemForm(idPrefix); });
    }
    // clear values and ensure starting state
    muboardResetRelatedItemForm(idPrefix);
    ...
    var acOptions = {
            ...
            afterUpdateElement: function(inputField, selectedListItem) {
                    ...
                    muboardSelectRelatedItem(objectType, idPrefix, inputField, selectedListItem);
            }
    };
    relationHandler.each(function(relationHandler) {
        if (relationHandler['prefix'] == (idPrefix + 'SelectorDoNew') && relationHandler['acInstance'] == null) {
            relationHandler['acInstance'] = new Ajax.Autocompleter(
                ...
                Zikula.Config['baseURL'] + 'ajax.php?module=MUBoard&func=getItemList',
                acOptions
            );
        }
    });
    ...
    $(idPrefix + 'SelectorDoNew').observe('click', function(e) {
        muboardInitInlineWindow(objectType, idPrefix + 'SelectorDoNew')
        e.stop();
    });
    ...
    itemIdsArr.each(function(existingId) {
        if (existingId) {
            ...
            $(elemPrefix).observe('click', function(e) {
                muboardInitInlineWindow(objectType, elemPrefix);
                e.stop();
            });
        }
    });
 ...
 */
function muboardSelectRelatedItem(objectType, idPrefix, inputField, selectedListItem)
{
    ...
    removeLink = Builder.node('a', {id: elemPrefix + 'Remove', href: 'javascript:muboardRemoveRelatedItem(\'' + idPrefix + '\', ' + newItemId + ');'}, 'remove');
    ...
    if (includeEditing == true) {
        ...
        $(elemPrefix + 'Edit').observe('click', function(e) {
            muboardInitInlineWindow(objectType, idPrefix + 'Reference_' + newItemId + 'Edit');
            e.stop();
        });
    }
    ...
    if (itemIds != '') {
        if ($F(idPrefix + 'Scope') == '0') {
            ...
            itemIdsArr.each(function(existingId) {
                if (existingId) {
                    muboardRemoveRelatedItem(idPrefix, existingId);
                }
            });
            itemIds = '';
        }
 ...
 */
function muboardInitInlineWindow(objectType, containerID)
{
    ...
    relationHandler.each(function(relationHandler) {
        ...
        if (relationHandler['prefix'] == containerID) {
            ...
            relationHandler['windowInstance'] = muboardCreateWindowInstance($(containerID), true);
        }
    });
    ...
    if (found === false) {
        ...
        newItem['windowInstance'] = muboardCreateWindowInstance($(containerID), true);
 ...
 */
function muboardCreateWindowInstance(containerElem, useIframe)
{
 ...
 */
function muboardRemoveRelatedItem(idPrefix, removeId)
{
    var itemIds = $F(idPrefix + 'ItemList');
    var itemIdsArr = itemIds.split(',');
 ...
 */
function muboardResetRelatedItemForm(idPrefix)
{
    // hide the sub form
    muboardToggleRelatedItemForm(idPrefix);
 ...
 */
function muboardToggleRelatedItemForm(idPrefix)
{
 ...
 */
function muboardCloseWindowFromInside(idPrefix, itemID)
{
